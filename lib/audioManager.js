...
// WebAudio-only, low-latency audio manager.
// Goals:
// - Single shared AudioContext (never recreated)
// - Decode once (AudioContext.decodeAudioData) and reuse buffers
// - No async logic inside click handlers
// - Mobile-safe: resume AudioContext on the first user gesture
...
export function preloadAudioBuffers() {
  ...
  preloadPromise = (async () => {
    ...
    // If preload fails, keep buffers null (no fallback requested).
    clickBuffer = null;
    robotBuffer = null;
  });
