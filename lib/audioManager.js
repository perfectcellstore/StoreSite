'use client';

/**
 * PRODUCTION-GRADE AUDIO MANAGER
 * 
 * Single global audio system for the entire application.
 * - ONE AudioContext (created once, reused forever)
 * - Buffers decoded ONCE on app start
 * - Zero async work in click handlers
 * - Mobile-safe (auto-unlocks on first user interaction)
 * - Works on ALL performance tiers (High/Mid/Low)
 */

let audioCtx = null;
let clickBuffer = null;
let robotBuffer = null;
let musicBuffer = null;

let preloadPromise = null;
let unlocked = false;
let initStarted = false;

let lastClickNode = null;
let lastRobotNode = null;
let musicNode = null;
let musicGainNode = null;
let isMusicPlaying = false;
let isMusicEnabled = true;
let isMusicLoaded = false;

/**
 * Initialize audio system immediately on app load.
 * This ensures buffers are ready before any clicks happen.
 */
export function initAudioSystem() {
  if (typeof window === 'undefined' || initStarted) return;
  initStarted = true;

  // Start preloading immediately (no user interaction needed for preload)
  preloadAudioBuffers();

  // Setup auto-unlock on FIRST user interaction (mobile requirement)
  const unlock = () => {
    unlockAudio();
  };

  // Use once: true to automatically remove after first call
  window.addEventListener('pointerdown', unlock, { capture: true, once: true, passive: true });
  window.addEventListener('touchstart', unlock, { capture: true, once: true, passive: true });
  window.addEventListener('click', unlock, { capture: true, once: true, passive: true });
}

function getCtx() {
  if (audioCtx) return audioCtx;
  try {
    const Ctx = window.AudioContext || window.webkitAudioContext;
    if (!Ctx) return null;
    audioCtx = new Ctx();
    return audioCtx;
  } catch {
    return null;
  }
}

function decodeWithCtx(ctx, arrayBuffer) {
  // Works across browsers (callback + promise forms)
  return new Promise((resolve, reject) => {
    try {
      const p = ctx.decodeAudioData(arrayBuffer, resolve, reject);
      if (p && typeof p.then === 'function') p.then(resolve).catch(reject);
    } catch (e) {
      reject(e);
    }
  });
}

export function preloadAudioBuffers() {
  if (typeof window === 'undefined') return null;
  if (preloadPromise) return preloadPromise;

  const ctx = getCtx();
  if (!ctx) {
    console.warn('[AudioManager] No AudioContext available');
    return null;
  }

  preloadPromise = (async () => {
    try {
      const [clickAb, robotAb] = await Promise.all([
        fetch('/sfx/click.wav', { cache: 'force-cache' }).then((r) => r.arrayBuffer()),
        fetch('/sfx/robot.wav', { cache: 'force-cache' }).then((r) => r.arrayBuffer()),
      ]);

      const [cb, rb] = await Promise.all([
        decodeWithCtx(ctx, clickAb),
        decodeWithCtx(ctx, robotAb),
      ]);

      clickBuffer = cb;
      robotBuffer = rb;
      console.log('[AudioManager] ✅ Audio buffers loaded successfully');
    } catch (err) {
      console.error('[AudioManager] ❌ Failed to load audio buffers:', err);
      clickBuffer = null;
      robotBuffer = null;
    }
  })();

  return preloadPromise;
}

// Resume/unlock audio (must be invoked from a user gesture on mobile).
// IMPORTANT: This should be called once; it never recreates the context.
export async function unlockAudio() {
  const ctx = getCtx();
  if (!ctx) {
    console.warn('[AudioManager] No AudioContext available for unlock');
    return false;
  }

  try {
    if (ctx.state === 'suspended') {
      await ctx.resume();
      console.log('[AudioManager] ✅ Audio unlocked, state:', ctx.state);
    }

    // Warm-up an inaudible tone so output path is ready
    try {
      const osc = ctx.createOscillator();
      const gain = ctx.createGain();
      osc.connect(gain);
      gain.connect(ctx.destination);
      const t0 = ctx.currentTime;
      osc.frequency.setValueAtTime(220, t0);
      gain.gain.setValueAtTime(0.0001, t0);
      osc.start(t0);
      osc.stop(t0 + 0.02);
    } catch {
      // ignore
    }

    unlocked = true;
  } catch (err) {
    console.error('[AudioManager] ❌ Failed to unlock audio:', err);
  }

  // Ensure buffers are decoded ASAP after the first gesture.
  // (Not in click handlers.)
  preloadAudioBuffers();

  return unlocked;
}

export function isAudioUnlocked() {
  return unlocked;
}

async function playBuffer(buffer, { volume = 0.35, kind = 'click' } = {}) {
  const ctx = getCtx();
  if (!ctx) {
    console.warn('[AudioManager] No AudioContext for playback');
    return;
  }
  
  if (!buffer) {
    console.warn(`[AudioManager] No buffer loaded for ${kind} sound`);
    return;
  }

  // Try to unlock/resume if needed
  if (ctx.state !== 'running') {
    console.log(`[AudioManager] Context suspended, attempting to unlock...`);
    await unlockAudio();
    
    // Check again after unlock attempt
    if (ctx.state !== 'running') {
      console.warn(`[AudioManager] AudioContext still not running (state: ${ctx.state}), skipping ${kind} sound`);
      return;
    }
  }

  try {
    const src = ctx.createBufferSource();
    src.buffer = buffer;

    const gain = ctx.createGain();
    gain.gain.setValueAtTime(volume, ctx.currentTime);

    src.connect(gain);
    gain.connect(ctx.destination);

    // Prevent stacking: stop previous node for same sound
    if (kind === 'click' && lastClickNode) {
      try { lastClickNode.stop(); } catch (e) { /* ignore */ }
    }
    if (kind === 'robot' && lastRobotNode) {
      try { lastRobotNode.stop(); } catch (e) { /* ignore */ }
    }

    if (kind === 'click') lastClickNode = src;
    if (kind === 'robot') lastRobotNode = src;

    src.start();
    console.log(`[AudioManager] ✅ Playing ${kind} sound`);
  } catch (err) {
    console.error(`[AudioManager] Failed to play ${kind} sound:`, err);
  }
}

// Synchronous-looking, but internally handles unlock if needed
export async function playClick() {
  await playBuffer(clickBuffer, { volume: 0.35, kind: 'click' });
}

export async function playRobot() {
  await playBuffer(robotBuffer, { volume: 0.4, kind: 'robot' });
}

// ==================== BACKGROUND MUSIC ====================

async function loadBackgroundMusic() {
  if (isMusicLoaded || typeof window === 'undefined') return;
  
  const ctx = getCtx();
  if (!ctx) return;

  try {
    console.log('[AudioManager] Loading background music...');
    const response = await fetch('/music/background.mp3', { cache: 'force-cache' });
    const arrayBuffer = await response.arrayBuffer();
    musicBuffer = await decodeWithCtx(ctx, arrayBuffer);
    isMusicLoaded = true;
    console.log('[AudioManager] ✅ Background music loaded');
  } catch (err) {
    console.error('[AudioManager] ❌ Failed to load background music:', err);
  }
}

export async function startBackgroundMusic() {
  const ctx = getCtx();
  if (!ctx) {
    console.warn('[AudioManager] No AudioContext for music');
    return;
  }

  // Unlock audio if needed
  if (ctx.state !== 'running') {
    await unlockAudio();
  }

  // Load music if not loaded
  if (!isMusicLoaded) {
    await loadBackgroundMusic();
  }

  if (!musicBuffer) {
    console.warn('[AudioManager] No music buffer loaded');
    return;
  }

  // Stop existing music if playing
  if (musicNode) {
    try {
      musicNode.stop();
    } catch (e) { /* ignore */ }
    musicNode = null;
  }

  try {
    musicNode = ctx.createBufferSource();
    musicNode.buffer = musicBuffer;
    musicNode.loop = true; // Loop the music

    musicGainNode = ctx.createGain();
    musicGainNode.gain.setValueAtTime(0.15, ctx.currentTime); // Quiet background music

    musicNode.connect(musicGainNode);
    musicGainNode.connect(ctx.destination);

    musicNode.start();
    isMusicPlaying = true;
    console.log('[AudioManager] ✅ Background music started');
  } catch (err) {
    console.error('[AudioManager] Failed to start background music:', err);
  }
}

export function stopBackgroundMusic() {
  if (musicNode) {
    try {
      musicNode.stop();
      console.log('[AudioManager] ⏹ Background music stopped');
    } catch (e) { /* ignore */ }
    musicNode = null;
  }
  isMusicPlaying = false;
}

export function toggleBackgroundMusic(enabled) {
  isMusicEnabled = enabled;
  
  if (enabled && !isMusicPlaying) {
    startBackgroundMusic();
  } else if (!enabled && isMusicPlaying) {
    stopBackgroundMusic();
  }
  
  // Save preference to localStorage
  if (typeof window !== 'undefined') {
    localStorage.setItem('musicEnabled', enabled ? 'true' : 'false');
  }
}

export function isMusicCurrentlyPlaying() {
  return isMusicPlaying;
}

export function getMusicEnabled() {
  // Load from localStorage on first call
  if (typeof window !== 'undefined') {
    const stored = localStorage.getItem('musicEnabled');
    if (stored !== null) {
      isMusicEnabled = stored === 'true';
    }
  }
  return isMusicEnabled;
}

// Begin initialization automatically (client-side only)
if (typeof window !== 'undefined') {
  initAudioSystem();
  
  // Auto-start music if enabled (will wait for user interaction)
  const musicEnabled = getMusicEnabled();
  if (musicEnabled) {
    // Wait for first user interaction before starting music
    const startMusic = async () => {
      await unlockAudio();
      await startBackgroundMusic();
    };
    
    window.addEventListener('click', startMusic, { once: true });
    window.addEventListener('touchstart', startMusic, { once: true });
  }
}
